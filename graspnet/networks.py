...
class CLIPGraspFusion(nn.Module):
    ...
    def _load_clip(self): # patch_size = 32, vision_width = 768, vision_layers = 12, vision_heads = 12, output_dim = 512, vocab_size = 49408
        ...
        self.clip = load_clip("ViT-B/32", device=self.device).to(self.device)
    ...
    def forward(self, bboxes, pos_bboxes, text, actions):
        ...
        # cosine similarity as logits 这句话描述的最像那个物体
        ...
        # encode grasp   MLP把grasp搞成embading  作为交叉注意力的Q
        ...
        # encode bbox positions  位置embading
        ...
        # cross attention
        cross_feat, attn_weights = self.cross_attn(q=grasp_feat, k=bbox_compound_feat,
                                                   v=fusion_feat)  # shape = [N, L', D]
        # cross_feat, attn_weights = self.cross_attn(q=grasp_feat, k=bbox_compound_feat, v=fusion_feat) # shape = [N, L', D]
        # probs 是输入的语言与8个物体的相似度。cross_feat16个抓取姿势的每个交叉注意特征?
