...
import torch
from tools.lm_select_box import lm_select_box
...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for image, targets in metric_logger.log_every(data_loader, print_freq, header, logger=logger):
...
def test_one_epoch(model: torch.nn.Module,
    ...
    tp1 = 0
    total = leng
    for samples, targets in tqdm(data_loader, desc="Processing lines"):

        samples = samples.to(device)
        captions = [t["caption"] for t in targets]
        true_bbox = [[t["boxes"] for t in targets]]
        true_bbox = true_bbox[0]
        true_bbox, index1 = del_none(true_bbox)
        total -= len(index1)
        ...
        with torch.no_grad():
            outputs = model(samples, captions=captions)
            tp = lm_select_box(outputs, captions, true_bbox, 0.5, index1)
            tp1 += tp
            # print(tp1)
...
def del_none(true_bbox):
    index1 = []
    for index, value in enumerate(true_bbox):
        if value.nelement() == 0:
            index1.append(index)
    if len(index1) == 0:
        return true_bbox, index1
    if len(index1) > 1:
        true_bbox = [j for i, j in enumerate(true_bbox) if i not in index1]
        return true_bbox, index1
    else:
        del true_bbox[index1[0]]
        return true_bbox, index1