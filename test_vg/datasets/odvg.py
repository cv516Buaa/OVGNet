...
class ODVGDataset(VisionDataset):
    ...
    def __getitem__(self, index: int):
        ...
        depth_path = rel_path[:7]+'.png'
        abs_path = os.path.join(self.root, rel_path)
        # depth_path = os.path.join(self.depth, depth_path) #limeng
        if not os.path.exists(abs_path):
            raise FileNotFoundError(f"{abs_path} not found.")
        # if not os.path .exists(depth_path):                   #limeng
        ...
        # depth = depth.convert('RGB')#limeng
        w, h = image.size
        if self.dataset_mode == "OD":
            anno = meta["detection"]
        ...
        if self.transforms is not None:
            image, target = self.transforms(image, target) #limeng
...
def make_coco_transforms(image_set, fix_size=False, strong_aug=False, args=None):
    ...
    normalize = T.Compose([
        T.ToTensor(),
        T.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225], [0.0139, 0.0139, 0.0139], [0.0151, 0.0151, 0.0151])
    ])
...
def build_odvg(image_set, args, datasetinfo):
    ...
    # depth_folder = datasetinfo["depth"]#limeng
    ...
    dataset = ODVGDataset(img_folder, ann_file, label_map, max_labels=args.max_labels,  #limeng depth folder
...
if __name__=="__main__":
    dataset_vg = ODVGDataset("path/GRIT-20M/data/", "path/GRIT-20M/depth/", "path/GRIT-20M/anno/grit_odvg_10k.jsonl",)
